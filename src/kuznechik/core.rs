...
impl Kuznechik {
    ...
    pub fn encrypt_cbc(&self, plaintext: Vec<u8>, iv: Vec<u8>) -> Vec<Vec<u8>> {
        ...
        let mut cipher_chunks: Vec<Vec<u8>> = Vec::new();
        ...
        for chunk in plaintext_chunks {
            ...
            // Save block
            cipher_chunks.push(encrypted_block.to_vec());
        }
...
mod tests {
    ...
    fn cbc_enc_test() {
        ...
        for (i, block) in kuznechik.encrypt_cbc(plaintext, iv).iter().enumerate() {
            assert_eq!(*block, ciphertext[BLOCK_SIZE*i..BLOCK_SIZE*(i+1)].to_vec())
        }
    }
